...
public class Calc {
  ...
  private int expn() {
    ...
    if (peek("**")) {
        consume("**");
        var exponent = expn(); // Right recursive call
        return (int) Math.pow(base, exponent); // Assumes base and exponent produce an integer result
    } else {
        return base;
    }
 }
...
class Lexer {
  ...
  private static final Pattern DOUBLE_STAR_RE = Pattern.compile("^\\*\\*"); //Double Star
  ...
  static List<Token> scan(String text) {
    ...
    while (text.length() > 0) {
      ...
      if ((lexeme = match(WS_RE, text)) != null) {
        ...
        tokens.add(new Token("**", lexeme)); //Double star
      }
      else {
